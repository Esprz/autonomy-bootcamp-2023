...
class DetectLandingPad:
    ...
    def run(self, image: np.ndarray) -> "tuple[list[bounding_box.BoundingBox], np.ndarray]":
        ...
        # * device
        # * verbose
        device = DetectLandingPad.__DEVICE
        conf_threshold = 0.7
        predictions = self.__model.predict(source=image, conf=conf_threshold, device=device, verbose=False)
        ...
        image_annotated = prediction.show()
        ...
        boxes_cpu = boxes_xyxy.detach()
        boxes_cpu = boxes_cpu.cpu()
